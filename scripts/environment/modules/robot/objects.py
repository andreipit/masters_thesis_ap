...
class RobotObjects():
    ...
    def add_objects(self, m: RobotModel):
        """ GameObject ID is called [Handle] """

        mesh_list = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        num_obj = len(mesh_list)
        random.shuffle(mesh_list)

        for object_idx in mesh_list: # in 1.2: 0,1,2,3,4 (obj_number == 5)
            #curr_mesh_file = os.path.join(os.getcwd(), 'objects', 'blocks', str(object_idx), '.obj')
            ...
            if ret_resp == 8: 
                print('Failed to add: ', object_idx)
            else:
                print('instantiated: ', object_idx)
            time.sleep(1.5)

    def add_objects2(self, m: RobotModel):
