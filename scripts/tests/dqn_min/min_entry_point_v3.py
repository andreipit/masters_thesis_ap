...
def start():
    env, NN, NN_frozen, opt, eps_ex_ex = create_objects()
    ...
    while True:
        ...
        if i % (SESSIONS * STEPS) == 0: # after ONE epoch
            ...
            torch.save(NN.state_dict(), os.path.join('tests','dqn_min', 'logs', 'saved_model_' + str(i) + '_r_' + str(int(np.mean(r_sessions))) +'.pt'))
